...
static off_t src_file_offset;

PG_FUNCTION_INFO_V1(read_xlog_rec);
PG_FUNCTION_INFO_V1(write_xlog_rec);

Datum
read_xlog_rec(PG_FUNCTION_ARGS)
{
	...
	read_record = (bytea*) palloc0(read_count + VARHDRSZ);
	SET_VARSIZE(read_record, read_count + VARHDRSZ);
	memcpy(VARDATA(read_record), xlog_rec_buffer, read_count);

	CloseTransientFile(wal_file_fd);
	pfree(tmp_buffer);
	pfree(xlog_rec_buffer);

	PG_RETURN_BYTEA_P(read_record);
}
